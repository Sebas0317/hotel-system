import { Calendar, Users, User, Dog, DollarSign, CheckCircle, AlertTriangle, Bed } from 'lucide-react';
...
export default function CheckinGuestForm({
  standalone, form, habitacionSeleccionada, selectedCalendarRange,
  onUpdateField, onCalendarSelect, onAgregarPersona, onActualizarPersona,
  ...
  return (
    <PantallaForm standalone={standalone} titulo="Registrar Huesped" desc="Completa los datos del huesped y la estadia" onVolver={onBack}>
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="md:col-span-1 p-5 bg-white border border-gray-200 rounded-xl shadow-sm">
              ...
              <div className="mt-3 pt-3 border-t border-green-200">
                <div className="flex flex-wrap gap-1.5">
                  {habitacionSeleccionada.amenidades.slice(0, 5).map((a, i) => {
                    ...
                    return (
                      <span key={i} className="inline-flex items-center gap-1 text-xs bg-white/80 backdrop-blur text-green-700 px-2 py-1 rounded-full border border-green-200">
          ...
          <button type="button" onClick={onBack} className="mt-4 w-full py-2 text-sm text-green-600 hover:text-green-700 font-medium border border-green-300 rounded-lg hover:bg-green-50">← Cambiar habitación</button>
        </div>

        <div className="md:col-span-3 space-y-6">
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2"><Calendar className="w-5 h-5 text-green-600" /> Fechas de la Estadía</CardTitle>
              <CardDescription>Selecciona check-in, número de noches y revisa el calendario</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  <label className="text-sm font-medium text-gray-700 block mb-2">Check-in</label>
                  <input type="date" value={form.checkIn} onChange={(e) => onUpdateField('checkIn', e.target.value)}
                ...
                <div>
                  ...
                  <input type="number" min="1" max="30" value={form.noches} onChange={(e) => onUpdateField('noches', parseInt(e.target.value) || 1)}
                    className="w-full px-4 py-3 rounded-lg border-2 border-gray-200 bg-gray-50 focus:bg-white focus:border-green-500 focus:ring-2 focus:ring-green-500/20 transition-all" />
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-700 block mb-2">Check-out</label>
                  <div className="px-4 py-3 bg-green-50 border-2 border-green-200 rounded-lg text-green-800 font-semibold">{form.checkOut || '—'}</div>
                </div>
              </div>
              <div className="mt-5">
                <RoomCalendar roomId={form.numero} roomNumero={habitacionSeleccionada?.numero} modo="range"
                  initialDates={selectedCalendarRange} onSelectDates={onCalendarSelect} />
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2"><Users className="w-5 h-5 text-green-600" /> Cantidad de Personas</CardTitle>
            ...
            <CardContent>
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <select value={form.adultos} onChange={(e) => onUpdateField('adultos', parseInt(e.target.value))}
                    className="w-full px-4 py-3 rounded-lg border-2 border-gray-200 bg-gray-50 focus:bg-white focus:border-green-500 focus:ring-2 focus:ring-green-500/20 transition-all">
                ...
                <div>
                  ...
                  <select value={form.ninos} onChange={(e) => onUpdateField('ninos', parseInt(e.target.value))}
                    className="w-full px-4 py-3 rounded-lg border-2 border-gray-200 bg-gray-50 focus:bg-white focus:border-green-500 focus:ring-2 focus:ring-green-500/20 transition-all">
                    {[0,1,2,3,4,5,6].map(n => <option key={n} value={n}>{n === 0 ? 'Sin niños' : `${n} niño${n > 1 ? 's' : ''}`}</option>)}
                  </select>
                </div>
              </div>
              {totalPersonas > 0 && (
                <p className="text-green-600 font-semibold bg-green-50 px-4 py-2 rounded-lg inline-block">Total: {totalPersonas} persona{totalPersonas > 1 ? 's' : ''}</p>
              )}
              <div className="mt-6 pt-6 border-t border-gray-100">
                <label className="flex items-center gap-3 cursor-pointer p-3 rounded-lg hover:bg-gray-50 transition-colors">
                  <input type="checkbox" checked={form.tieneMascota} onChange={(e) => onUpdateField('tieneMascota', e.target.checked)}
                    className="w-5 h-5 text-green-600 rounded border-gray-300 focus:ring-green-500" />
                  <span className="flex items-center gap-2 text-gray-700"><Dog className="w-5 h-5" /> Mascota (sin costo)</span>
                </label>
                {form.tieneMascota && (
                  <div className="mt-3 ml-8">
                    <input type="text" placeholder="Nombre de la mascota" value={form.nombreMascota}
                      onChange={(e) => onUpdateField('nombreMascota', e.target.value)}
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2"><User className="w-5 h-5 text-green-600" /> Datos del Huésped</CardTitle>
            ...
            <CardContent>
              <div className="space-y-4">
                <div>
                  ...
                  <input type="text" placeholder="Ej: Juan García" value={form.huesped}
                    onChange={(e) => onUpdateField('huesped', e.target.value)}
                    className="w-full px-4 py-3 rounded-lg border-2 border-gray-200 bg-gray-50 focus:bg-white focus:border-green-500 focus:ring-2 focus:ring-green-500/20 transition-all" autoFocus />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm font-medium text-gray-700 block mb-2">Documento</label>
                    <input type="text" placeholder="Cédula" value={form.documento} onChange={(e) => onUpdateField('documento', e.target.value)}
                      className="w-full px-4 py-3 rounded-lg border-2 border-gray-200 bg-gray-50 focus:bg-white focus:border-green-500 focus:ring-2 focus:ring-green-500/20 transition-all" />
                  </div>
                  <div>
                    <label className="text-sm font-medium text-gray-700 block mb-2">Teléfono</label>
                    <input type="tel" placeholder="310 123 4567" value={form.telefono} onChange={(e) => onUpdateField('telefono', e.target.value)}
                      className="w-full px-4 py-3 rounded-lg border-2 border-gray-200 bg-gray-50 focus:bg-white focus:border-green-500 focus:ring-2 focus:ring-green-500/20 transition-all" />
                  </div>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-700 block mb-2">Correo electrónico</label>
                  <input type="email" placeholder="juan@email.com" value={form.email} onChange={(e) => onUpdateField('email', e.target.value)}
                    className="w-full px-4 py-3 rounded-lg border-2 border-gray-200 bg-gray-50 focus:bg-white focus:border-green-500 focus:ring-2 focus:ring-green-500/20 transition-all" />
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-700 block mb-2">Observaciones</label>
                  <textarea placeholder="Notas especiales..." value={form.observaciones} onChange={(e) => onUpdateField('observaciones', e.target.value)}
                    className="w-full px-4 py-3 rounded-lg border-2 border-gray-200 bg-gray-50 focus:bg-white focus:border-green-500 focus:ring-2 focus:ring-green-500/20 transition-all" rows={2} />
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          {totalPersonas > 1 && (
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2"><Users className="w-5 h-5 text-green-600" /> Datos de personas adicionales</CardTitle>
                <CardDescription>Información de las demás personas en la habitación</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="mb-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
                  <label className="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" checked={form.usarMismoContacto} onChange={(e) => onUpdateField('usarMismoContacto', e.target.checked)}
                ...
                <div className="flex items-center justify-end mb-4">
                  <button type="button" onClick={onAgregarPersona} className="text-sm text-green-600 hover:text-green-700 font-medium flex items-center gap-1">
                    <span className="text-lg">+</span> Agregar persona
                  </button>
                </div>
                {form.personasAdicionales.length === 0 ? (
                  <p className="text-sm text-gray-500 text-center py-4">Haz clic en "Agregar persona" para registrar más personas</p>
                ) : (
                  form.personasAdicionales.map((persona, i) => (
                    <div key={i} className="mb-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                      <div className="flex justify-between items-center mb-3">
                        ...
                        <button type="button" onClick={() => onActualizarPersona(i, null)}
                          className="text-red-500 hover:text-red-700 text-sm">Eliminar</button>
                      </div>
                      <div className="grid grid-cols-2 gap-3">
                        <input type="text" placeholder="Nombre completo" value={persona.nombre || ''}
                          onChange={(e) => onActualizarPersona(i, 'nombre', e.target.value)}
                        ...
                        <input type="text" placeholder="Documento" value={persona.documento || ''}
                          onChange={(e) => onActualizarPersona(i, 'documento', e.target.value)}
                          className="px-3 py-2 rounded-lg border border-gray-200 focus:border-green-500 focus:ring-2 focus:ring-green-500/20 text-sm" />
                      </div>
                      <div className="mt-3 pt-3 border-t border-gray-200">
                        <p className="text-xs text-gray-500 mb-2">Datos de contacto (opcional)</p>
                        <div className="grid grid-cols-2 gap-3">
                          <input type="email" placeholder="Correo electrónico" value={persona.email || ''}
                            onChange={(e) => onActualizarPersona(i, 'email', e.target.value)} disabled={form.usarMismoContacto}
                          ...
                          <input type="tel" placeholder="Teléfono" value={persona.telefono || ''}
                            onChange={(e) => onActualizarPersona(i, 'telefono', e.target.value)} disabled={form.usarMismoContacto}
                            className="px-3 py-2 rounded-lg border border-gray-200 focus:border-green-500 focus:ring-2 focus:ring-green-500/20 text-sm disabled:bg-gray-100 disabled:text-gray-400" />
                        </div>
                      </div>
                    </div>
          ...
          <Card className="border-green-200 bg-gradient-to-br from-green-50 to-green-100/50">
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-green-800"><DollarSign className="w-5 h-5" /> Resumen de Tarifa</CardTitle>
            ...
            <div className="px-6 pb-6">
              <div className="pt-4 border-t border-green-200 flex justify-between items-center">
                ...
                <span className="text-3xl font-bold text-green-600">{(precioAdultos + precioNinos + precioMascota).toLocaleString('es-CO')} COP</span>
              </div>
            </div>
          </Card>
