...
export default function PantallaCheckin({ onNav }) {
  ...
  return (
    <PantallaForm titulo="🌿 Registrar Huésped" desc="Selecciona una habitación disponible y registra los datos del huésped" onVolver={() => onNav('menu')}>
      ...
      {form.numero && (
        <div className="selected-room-preview mb-6 p-6 bg-gradient-to-r from-green-50 to-green-100 border-2 border-green-500 rounded-2xl shadow-lg">
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1">
              <p className="text-xs font-bold text-green-600 uppercase tracking-wide mb-2">Habitación Seleccionada</p>
              <p className="text-4xl font-extrabold text-green-900 mb-1">#{rooms.find(r => r.id === form.numero)?.numero}</p>
              <p className="text-xl font-bold text-green-700 mb-3">{rooms.find(r => r.id === form.numero)?.tipo}</p>
              <div className="flex flex-wrap gap-2 mb-3">
                <span className="text-sm text-green-600 bg-green-200 px-2 py-1 rounded">🛏️ {rooms.find(r => r.id === form.numero)?.camas}</span>
                <span className="text-sm text-green-600 bg-green-200 px-2 py-1 rounded">👥 {rooms.find(r => r.id === form.numero)?.capacidad} personas</span>
                <span className="text-sm text-green-600 bg-green-200 px-2 py-1 rounded">📍 {rooms.find(r => r.id === form.numero)?.piso === 0 ? 'Cabañas' : `Piso ${rooms.find(r => r.id === form.numero)?.piso}`}</span>
              </div>
              {rooms.find(r => r.id === form.numero)?.amenidades && (
                <div className="flex flex-wrap gap-1.5">
                  {rooms.find(r => r.id === form.numero)?.amenidades.slice(0, 4).map((a, i) => (
            ...
            <div className="text-right flex flex-col items-end">
              ...
              <p className="text-sm text-green-500 font-medium">COP/noche</p>
              <button 
                type="button"
