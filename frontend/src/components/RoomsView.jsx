...
import {
  LayoutDashboard, Home, Check, Bed, Calendar, Clock,
  Search, X, Grid3X3, List, Building2
} from 'lucide-react';
...
export default function RoomsView() {
  const { rooms, refresh, handleSelectRoom, selectedRoomId, inlineToast, setInlineToast } = useOutletContext();
  ...
  const handleFilter = useCallback((newFiltro) => { setFiltro(newFiltro); }, []);
  const handleViewMode = useCallback((newMode) => { setViewMode(newMode); }, []);
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50/80">
            <tr className="text-left text-gray-500 border-b border-gray-200/60">
              ...
              <th className="px-4 py-2 font-semibold hidden md:table-cell">Huesped</th>
      ...
      {Object.entries(grupos).map(([piso, roomsInPiso]) => (
        <div key={piso} className="mb-8">
          <h3 className="text-sm font-semibold text-gray-600 uppercase tracking-wider mb-4 flex items-center gap-2">
            ...
            {piso === '0' ? <><Home className="w-4 h-4 inline mr-1" /> Cabanas</> : <><Building2 className="w-4 h-4 inline mr-1" /> Piso {piso}</>}
            <span className="w-8 h-0.5 bg-gradient-to-r from-gray-300/60 to-transparent inline-block"></span>
      ...
      <div className="relative">
        <div className="flex items-center gap-2 text-sm text-gray-500 mb-4">
          ...
          {selectedRoomId && (
            <>
              <span>›</span>
              <span className="text-green-600 font-medium">Habitacion seleccionada</span>
            </>
          )}
        </div>
            ...
            <div className="hidden xl:block">
              <div className="sticky top-[130px] bg-white/90 backdrop-blur-sm rounded-xl shadow-lg border border-gray-200/60 overflow-hidden">
                <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 bg-gray-50">
                  <h3 className="font-semibold text-gray-900">Habitacion #{selectedRoom.numero}</h3>
                  <button
