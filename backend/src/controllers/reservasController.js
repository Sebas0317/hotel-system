...
const reservasController = {
  ...
  async getByRoom(req, res) {
    try {
      ...
      logger.error({ err: e, roomId: req.params.roomId }, 'Error getting reservas by room');
      res.status(500).json({ error: e.message });
    }
  },
  ...
  async update(req, res) {
    try {
      ...
      logger.error({ err: e, reservaId: req.params.id }, 'Error updating reserva');
      res.status(500).json({ error: e.message });
    }
  },
  ...
  async cancel(req, res) {
    try {
      ...
      logger.error({ err: e, reservaId: req.params.id }, 'Error canceling reserva');
      res.status(500).json({ error: e.message });
    }
  },
  ...
  async checkIn(req, res) {
    try {
      ...
      logger.error({ err: e, reservaId: req.params.id }, 'Error checking in reserva');
      res.status(500).json({ error: e.message });
    }
  },
  ...
  async checkOut(req, res) {
    try {
      ...
      logger.error({ err: e, reservaId: req.params.id }, 'Error checking out reserva');
      res.status(500).json({ error: e.message });
    }
  },
  ...
  async getAvailability(req, res) {
    try {
      ...
      if (!roomId || !startDate || !endDate) {
        return res.status(400).json({ error: 'roomId, startDate, endDate required' });
      }
      const reservas = await getReservas();
